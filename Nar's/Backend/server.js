...
app.get('/order/:orderId', (req, res) => {
  ...
  const orderSql = `
    SELECT o.*, op.name, op.product_id, op.quantity, op.price
    FROM orders o
...
app.post('/submit-ratings/:orderId', (req, res) => {
  ...
  db.query(checkOrderSql, [orderId, req.session.user.id], (err, orderResult) => {
    ...
    db.query(fetchProductsSql, [orderId], (err, productsResult) => {
      ...
      const ratingPromises = Object.entries(ratings).map(([index, rating]) => {
        const productId = productsResult[index].product_id;
        return new Promise((resolve, reject) => {
  ...
  db.query(checkColumnSql, (err, result) => {
    ...
    db.query(sql, [req.session.user.id], (err, results) => {
      if (err) {
        ...
        return res.status(500).json({ error: 'An error occurred while fetching order history', details: err.message });
      }
