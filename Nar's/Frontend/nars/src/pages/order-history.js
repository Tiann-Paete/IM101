...
const OrderHistory = () => {
  ...
  const handleRatingSubmit = async ({ ratings, feedback }) => {
    try {
      await axios.post(`http://localhost:8000/submit-ratings/${selectedOrder.id}`, { ratings, feedback }, { withCredentials: true });
      setShowRatingModal(false);
      fetchOrderHistory(); // Refresh the order history to show updated ratings
    } catch (error) {
